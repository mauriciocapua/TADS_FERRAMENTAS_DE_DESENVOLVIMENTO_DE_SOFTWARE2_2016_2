...
class AnotacaoDAO:
    def listar(self, usuario):
        objConexao = Conexao()
        objConexao.cur.execute("SELECT * FROM anotacao where id_usuario = (%s);",
                               [usuario.id])
        lista = []
        ...
        while (i < linhasRetornadas):
            ...
            lista.append(Anotacao(registro[0], registro[1], registro[2]))
            i += 1
        objConexao.fechar()
        return lista
    ...
    def inserir(self, anotacao, usuario):
        # abri cursor

        objConexao = Conexao()

        # sql
        objConexao.cur.execute("INSERT INTO anotacao(titulo, descricao, id_usuario) VALUES (%s,%s,%s)",
                               [anotacao.titulo, anotacao.descricao, usuario.id])
    ...
    def excluir(self, anotacao):
        # criar a conexao
        objConexao = Conexao()

        # sql
        objConexao.cur.execute("DELETE FROM anotacao WHERE id = (%s);", [anotacao.id])
    ...
    def update(self, anotacao):
        ...
        objConexao.cur.execute("UPDATE anotacao SET titulo = (%s), descricao = (%s) WHERE id = (%s);",
                               [anotacao.titulo, anotacao.descricao, anotacao.id])
        objConexao.con.commit()
        objConexao.fechar()
